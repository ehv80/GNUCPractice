...
/*  
*   Definición de la FUNCIÓN: 	inverts( unsigned x, int p, int n )
*  
*   Regresa x con los n bits que principian en la posición p 
*   invertidos (esto es 1 cambiando a 0 y viceversa) 
...
int main( int c, char * argv[] )
{
	...
	int result = invert( x, 3, 2);

	printf("\nSe aplica la función   invert  a x ! y retorna: %d \n", result);
	printf("\nThe   invert   function is applied to the x ! and return %d  \n", result);
